...
const OnlineGameBoard = () => {
  ...
  const handleToggleEliminate = async (id: string) => {
    ...
    if (guessingMode) {
      if (myEliminated.has(id)) return; // item yang sudah ditutup tidak bisa dipilih saat mode tebak
      setPendingGuess(id);
      return;
    }
            ...
            return (
              <button
                ...
                className={`relative clip-angle p-0.5 transition-all duration-300 group animate-slide-up
                  ...
                  : isEliminated ? `bg-border/30 scale-[0.97] ${guessingMode ? 'cursor-not-allowed' : ''}`
